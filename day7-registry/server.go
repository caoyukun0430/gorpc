...
// as long as we implement ServeHTTP method, server implement Handler interface
// type Handler interface {
//     ServeHTTP(w ResponseWriter, r *Request)
// }

// day 5 the advantage of support HTTP is that RPC server only listen the defaultRPCPath, but we can support/listen
// on other path e.g. debug path for telemetry, metrics ... by registering MORE servers!

// HandleHTTP registers an HTTP handler for RPC messages on rpcPath.
// It is still necessary later to invoke http.Serve(l net.Listener, handler http.Handler) error:
// This function is more flexible and allows you to use a custom net.Listener.
// You can create a listener on a specific network address and then pass it to http.Serve().
...
// day5 ServeHTTP implements an http.Handler that answers RPC requests.
// // ResponseWriter: construct an HTTP response. It provides methods to:
// Set response headers: Using the Header() method.
// Write the response body: Using the Write([]byte) method.
// Set the HTTP status code: Using the WriteHeader(statusCode int) method.
func (server *Server) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	//  http.Hijacker interface, which allows an HTTP handler to take over the underlying network connection, e.g. tcp
	// After hijacking, you can directly interact with the conn to handle the connection as needed. This is useful for protocols like WebSockets or custom protocols that need to take over the connection after the initial HTTP handshake
	...
	// HTTP Response Structure HTTP version, status code, and a reason phrase: HTTP/1.1 200 OK
	// why not use w.Write
	// you are using io.WriteString to write directly to the hijacked connection. If you want to use http.ResponseWriter instead, it won't work after hijacking the connection because the http.ResponseWriter is no longer valid once the connection is hijacked. The Hijack method takes over the underlying network connection, and you must interact with the net.Conn directly.
