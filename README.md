...
## Day5 - HTTP prtocol support

1. HTTP CONNECT is primarily used to create a secure tunnel for HTTPS traffic, ensuring that the communication between the browser and the target site remains private. We ultize it
for establishing HTTP then RPC. Server implements HTTP Handler interface, and accepts only HTTP CONNECT requests, then hijack the TCP connection underneath to use as RPC connection
between the client and server. Before that, it's simply Server Accept to use the TCP listener for communication. Basically, the server and client has wrapped the RPC inside the HTTP
server (ServeHTTP) and client (NewHTTPClient)
