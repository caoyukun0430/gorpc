...
const (
	...
	defaultDebugPath = "/debug/methods"
)
...
// day5 as the server is ready to receive HTTP req and send HTTP response
// now the client should about to send HTTP req
// HTTP Request Structure HTTP method (e.g., GET, POST, CONNECT), the resource path, and the HTTP version.
// GET /index.html HTTP/1.1
// ALSO this belongs type newClientFunc due to the args
func NewHTTPClient(conn net.Conn, opt *Option) (*Client, error) {
	_, _ = io.WriteString(conn, fmt.Sprintf("CONNECT %s HTTP/1.0\n\n", defaultRPCPath))

	// Require successful HTTP response before switching to RPC protocol.
	// The http.ReadResponse function needs to know the context of the request to correctly parse the response. By providing &http.Request{Method: "CONNECT"}, you're informing the function that the response it should expect is for a CONNECT request.
...
// day 5 DialHTTP connects to an HTTP RPC server at the specified network address
// listening on the default HTTP RPC path.
// NewHTTPClient is responsible to send HTTP CONNECT, the following RPC is handled by the client
